...
impl Iterator for NSFastIterator {
    ...
    fn next(&mut self) -> Option<id> {
        if self.idx >= self.len {
            self.len = unsafe {
                msg_send![self.object, countByEnumeratingWithState:&mut self.state objects:self.buffer count:16]
            };
            self.idx = 0;
        }
        ...
        if self.idx < self.len {
            let object = unsafe {
                let objects: &[id] = slice::from_raw_parts(self.state.items_ptr, self.len);
                objects[self.idx]
            };        
